...
import uk.gov.justice.digital.hmpps.personintegrationapi.common.client.request.DistinguishingMarkRequest
...
class DistinguishingMarksServiceTest {
  ...
  inner class UpdateDistinguishingMark {
    @BeforeEach
    fun beforeEach() {
      whenever(prisonApiClient.updateDistinguishingMark(DISTINGUISHING_MARK_REQUEST, PRISONER_NUMBER, 1))
        .thenReturn(ResponseEntity.ok(DISTINGUISHING_MARK))
    }

    @Test
    fun `can update a distinguishing mark by id`() {
      val response = underTest.updateDistinguishingMark(DISTINGUISHING_MARK_REQUEST, MARK_ID, SOURCE_NOMIS)
      assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
      assertThat(response.body).isEqualTo(DISTINGUISHING_MARK)
    }
    ...
    fun `throws an exception if the source system is not supported`() {
      assertThrows<IllegalArgumentException> {
        underTest.updateDistinguishingMark(DISTINGUISHING_MARK_REQUEST, MARK_ID, "UNKNOWN_SOURCE")
      }
    }
    ...
    fun `throws an exception if the mark id is in an invalid format`(markId: String) {
      assertThrows<IllegalArgumentException> {
        underTest.updateDistinguishingMark(DISTINGUISHING_MARK_REQUEST, markId, SOURCE_NOMIS)
      }
    }
  }
  ...
  inner class CreateDistinguishingMark {
    @BeforeEach
    fun beforeEach() {
      whenever(prisonApiClient.createDistinguishingMark(MULTIPART_FILE, DISTINGUISHING_MARK_REQUEST, PRISONER_NUMBER))
        .thenReturn(ResponseEntity.ok(DISTINGUISHING_MARK))
    }

    @Test
    fun `can create a distinguishing mark`() {
      val response = underTest.createDistinguishingMark(MULTIPART_FILE, DISTINGUISHING_MARK_REQUEST, PRISONER_NUMBER, SOURCE_NOMIS)
      assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
      assertThat(response.body).isEqualTo(DISTINGUISHING_MARK)
    }
    ...
    fun `throws an exception if the source system is not supported`() {
      assertThrows<IllegalArgumentException> {
        underTest.createDistinguishingMark(MULTIPART_FILE, DISTINGUISHING_MARK_REQUEST, PRISONER_NUMBER, "UNKNOWN_SOURCE")
      }
    }
  }
  ...
  private companion object {
    ...
    val DISTINGUISHING_MARK_REQUEST = DistinguishingMarkRequest(
      bodyPart = "TORSO",
      markType = "SCAR",
      side = "L",
      partOrientation = "UPP",
