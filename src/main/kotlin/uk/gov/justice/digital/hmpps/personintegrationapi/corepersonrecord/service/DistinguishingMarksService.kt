...
class DistinguishingMarksService(
  ...
  fun getDistinguishingMarks(prisonerNumber: String, sourceSystem: String): ResponseEntity<List<DistinguishingMarkDto>> = when (sourceSystem.toSourceSystem()) {
  ...
  fun updateDistinguishingMark(request: DistinguishingMarkRequest, markId: String, sourceSystem: String): ResponseEntity<DistinguishingMarkDto> = when (sourceSystem.toSourceSystem()) {
    NOMIS -> {
      val (prisonerNumber, sequenceId) = parseMarkId(markId)
  ...
  fun createDistinguishingMark(
    file: MultipartFile?,
    request: DistinguishingMarkRequest,
  ...
  fun addDistinguishingMarkImage(file: MultipartFile, markId: String, sourceSystem: String): ResponseEntity<DistinguishingMarkDto> = when (sourceSystem.toSourceSystem()) {
    NOMIS -> {
      val (prisonerNumber, sequenceId) = parseMarkId(markId)
